	...
	$.getJSON("json/stats.json", function(result) {
		...
		for (var i = 0; i < result.servers.length; i++) {
			...
			if (!result.servers[i].online4 && !result.servers[i].online6) {
				...
				// Network
				var netstr = "";
				if(result.servers[i].network_rx*8 < 1024)
					netstr += result.servers[i].network_rx.toFixed(0)*8 + "B";
				else if(result.servers[i].network_rx*8 < 1024*1024)
					netstr += (result.servers[i].network_rx/1024).toFixed(0)*8 + "K";
				else
					netstr += (result.servers[i].network_rx/1024/1024).toFixed(1)*8 + "M";
				netstr += " | "
				if(result.servers[i].network_tx*8 < 1024)
					netstr += result.servers[i].network_tx.toFixed(0)*8 + "B";
				else if(result.servers[i].network_tx*8 < 1024*1024)
					netstr += (result.servers[i].network_tx/1024).toFixed(0)*8 + "K";
				else
					netstr += (result.servers[i].network_tx/1024/1024).toFixed(1)*8 + "M";
